matrixFont v1.3. Руководство пользователя
=========================================
...
- отображение в редакторе: пиксельная сетка, шахматный фон;
...
| -------------------------- | -------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Переместить вверх          | `Правка > Переместить вверх`           | обменивает местами текущий символ и предыдущий                                                            |
| Переместить вниз           | `Правка > Переместить вниз`            | обменивает местами текущий символ и следующий за ним                                                      |
...
| Центрирование              | `Эффекты > Центрировать > ...`         | перемещает символ так, чтобы он оказался посередине в выбранном направлении                               |
...
Команда `Правка > Импорт изображения …` дает возможность создать символ из произвольного изображения в одном из широко используемых графических форматов. Также можно просто перетащить графический файл в редактор. При этом в текущий символ будет вставлено бинаризированное изображение. Порог черного можно задать в настройках приложения. Изображение импортируется попиксельно. Если размеры изображения больше размеров символа, будет вставлена только часть изображения, начиная с левого верхнего угла.
...
Команда `Файл > Образец текста` показывает окно с образцом текста для визуальной оценки создаваемого шрифта. Образец можно масштабировать колесом мыши, текущий масштаб отображается в заголовке окна.
...
Изображение предпросмотра может обновляться автоматически при изменении символов в редакторе. Это можно задать в настройках — флажок `Обновлять образец текста предпросмотра автоматически`. Если опция отключена, можно обновить образец нажатием кнопки `Обновить образец текста`.
...
1. Скопируйте файл шаблона `*.pot` (каталог [bin/lang](../bin/lang)) и замените расширение локали на [свое](https://wiki.freepascal.org/Language_Codes), например, `*.uk.po`.
2. Откройте свой файл `*.uk.po` в [Virtaal](https://virtaal.translatehouse.org) или [Poedit](https://poedit.net).
3. Осуществите перевод с русского на свой родной язык и сохраните переведенный файл.
4. Поделитесь Вашим переводом в автором и он появится в следующей сборке приложения.
5. Чтобы иметь возможность выбрать новый перевод в настройках приложения добавьте в `languages.ini` определение для своего перевода:
   - увеличьте значение `Count` на единицу (`Count` показывает число доступных локализаций),
   - добавьте строку вида `L-<index>=<locale>,<description>`, например,  `L-1=uk, Ukrainian - Українська`,
   - обратите внимание, символы после `=` до первой запятой должны совпадать с суффиксом локали (регистр не имеет значения).
